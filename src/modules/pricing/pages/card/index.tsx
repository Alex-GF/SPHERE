...
export default function CardPage() {
    ...
    useEffect(() => {
      ...
      fetch('src/assets/'+pricingYamlPath).then(async response => {
        ...
        const contentLength = response.headers.get("content-length") ? parseInt(response.headers.get("content-length")??"0") : null;
    ...
    const handleInputDate = (e: React.ChangeEvent<HTMLInputElement>) => {
      const newPricingData = fullPricingData?.filter((entry) => new Date(entry.date) >= new Date(e.target.value));
      if (newPricingData) {
        ...
        <>
          <Box flex={1}>
            ...
            <Typography variant="body1" paragraph>
              ...
              review code like a pro, track bugs and features, power your CI/CD and DevOps workflows, 
              and secure code before you commit it.
            </Typography>
            {pricing && (
          ...
          <Box sx={{ minWidth: '33.3%'}}>
            { currentPricing && pricing && (
              <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
                <Stats pricingDetails={pricingDetails} currentPricing={currentPricing} pricing={pricing} />
              </Paper>
            )} 
