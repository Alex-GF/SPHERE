...
interface CollectionAnalyticsModalProps {
    open: boolean;
    onClose: () => void;
    collectionData: CollectionAnalytics | undefined;
}
          ...
          {/* <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 3, maxHeight: "80vh", overflowY: "auto" }}>
            <Box>
              ...
              <LineChart
                width={500}
                height={300}
                series={[{ data: collectionData?.map((entry) => entry.analytics.configurationSpaceSize).reverse() ?? [], area: false, showMark: true, label: 'Available Subscriptions' }]}
            ...
            <Box>
              ...
              <LineChart
                ...
                height={300}
                series={[
                        { data: collectionData?.map((entry) => entry.analytics.minSubscriptionPrice).reverse() ?? [], area: false, showMark: true, label: 'Min Price' },
                        { data: collectionData?.map((entry) => entry.analytics.maxSubscriptionPrice).reverse() ?? [], area: false, showMark: true, label: 'Max Price' }]}
                xAxis={[{ scaleType: 'point', data: collectionData?.map((entry) => new Date(entry.extractionDate).getFullYear().toString()).reverse() ?? [] }]}
                />
            </Box>
            ...
            <Box>
              ...
              <LineChart
                width={500}
                height={300}
                series={[{ data: collectionData?.map((entry) => entry.analytics.numberOfPlans).reverse() ?? [], area: false, showMark: true, label: 'Plans' }]}
                xAxis={[{ scaleType: 'point', data: collectionData?.map((entry) => new Date(entry.extractionDate).getFullYear().toString()).reverse() ?? [] }]}
                />
            </Box>
            ...
            <Box>
              ...
              <LineChart
                width={500}
                height={300}
                series={[{ data: collectionData?.map((entry) => entry.analytics.numberOfFeatures).reverse() ?? [], area: false, showMark: true, label: 'Features' }]}
                xAxis={[{ scaleType: 'point', data: collectionData?.map((entry) => new Date(entry.extractionDate).getFullYear().toString()).reverse() ?? [] }]}
                />
            </Box>
